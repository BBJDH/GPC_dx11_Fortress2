...
namespace Engine::Physics
{
    ...
    bool Component<Circle>::Collide(Component<Quadrangle> const & component) const
    {
        ...
        //사각형 충돌할때 min max와 같음

        if((min.x <= Center.x and Center.x <= max.x) or (min.y <= Center.y and Center.y <= max.y)) //예외아닌범위일때
        {
            min = Point(min.x - Radius, min.y - Radius); //사각형 확장
            max = Point(max.x + Radius, max.y + Radius);

            return (min.x <= Center.x and Center.x <= max.x) and (min.y <= Center.y and Center.y <= max.y); //커진 사각형과 원의 중심과의 충돌 검사
        }
        else
        {
