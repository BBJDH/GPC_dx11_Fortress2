...
namespace Engine::Rendering::Pipeline
{
    ...
    namespace String
    {
        void Render(HFONT const hFont, LPCSTR const string, COLORREF const color, SIZE const & size, POINT const & center)
        {
            ...
            Surface->Release();  //release dc를 하면 렌더타겟 결합이 해제된다 렌더타겟을 다시 설정해야한다
            ...
            {
                {
                    ...
                    Descriptor.Flags             = DXGI_SWAP_CHAIN_FLAG_GDI_COMPATIBLE; //호환가능한, GDI와 호환 가능하게
                ...
                }
                {
                    ...
                    }
                    {
                        ...
                        MUST(SwapChain->GetBuffer(0, IID_PPV_ARGS(&Texture2D)));
                        {
                            ...
                            MUST(Texture2D->QueryInterface(IID_PPV_ARGS(&Surface))); //텍스쳐2d에게 surface 인터페이스 유무를 물어본다
                            {
                                HDC hDC = HDC();

                                MUST(Surface->GetDC(false, &hDC)); //이전내용을 없애지 않게 설정
                                {
