...
Scene * S_Battle::update_scene()
{
    ...
    case S_Battle::State::Playing:
    {
        ...
        _Input_manager->input(_Tank->tanks, _Missile->missiles,Engine::Time::Get::Delta());
...
void S_Battle::dispose_objects()
{
    ...
    _Patterns->assign_pattern(delta);
    _Physics_manager->ballistics(_Tank->tanks, _Missile->missiles, delta);
...
{
    _CAM->update();
    _Map_manager->render_map(_Patterns->patterns); //배경 -> 패턴 -> 맵
