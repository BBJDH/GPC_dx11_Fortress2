...
Missile_Tank::Missile_Tank(_float2 const& pos, unsigned const width, unsigned const height,
	std::string const& player_name, std::string const& tank_name, Color const color)
	:Tank(pos, width, height, {20, 45}, player_name, tank_name, Tank::Tank_Type::Missile_Tank, color)
{
...
void Missile_Tank::ani_render(float const delta)
{
	ani_playtime += delta;
